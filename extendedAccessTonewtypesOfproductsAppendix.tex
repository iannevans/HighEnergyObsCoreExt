...
The {\tt ivoa.response} table uses a specific {\em resp\_dataproduct\_type\/} column that is compliant with the proposed response type vocabulary (\url{https://www.ivoa.net/rdf/response-type/}) and has its own {\em resp\_obs\_publisher\_did\/} column as well, in order to distinguish which response matches  which ObsCore data set in the {\tt ivoa.response} table. 

\subsection*{Preliminary Proposal for a Response Table {\tt ivoa.response}}
...
\caption{Preliminary proposal for a standard response table. With appropriate further study the columns identified herein could form the basis for a recommended base set of columns for an  {\tt ivoa.response} table.}
...
\subsubsection*{Retrieve {\bf psf} {\bf response-function}s for {\em obs\_id\/} and position criteria}
...
When using the response table defined in Table \ref{tab:response_table}, we can join the {\tt ivoa.obscore} table with the {\tt ivoa.response} table on one of the two keys ({\em obs\_id\/} and {\em obs\_publisher\_did\/}), for example: 
{\small
\begin{verbatim}
SELECT o.obs_publisher_did, o.obs_id, s_ra, s_dec, s_fov, t_min, t_max, 
energy_min, energy_max, access_url, access_format, 
resp_publisher_did, r.obs_publisher_did, r.obs_id, resp_access_url,
resp_access_format, resp_energy_max, resp_energy_min
...
\subsubsection*{Retrieve {\bf edisp} {\bf response-function}s for data sets constrained by {\em obs\_id} or position and by {\em scan\_mode}}
{\small
\begin{verbatim}
SELECT o.obs_publisher_did, o.obs_id, s_ra, s_dec, s_fov, t_min, t_max, 
...
FROM ( SELECT o.obs_publisher_did, o.obs_id, scan_mode 
     ...
     AND (o.obs_id = '1976')
     OR CONTAINS(POINT(s_ra, s_dec), CIRCLE, 83.6324, +22.0174, 0.083333) = 1)
     as s
JOIN ivoa.response ON 
resp.obs_id = s.obs_id
WHERE (resp_product_type = 'edisp')
% s is the list of observations satisfying position or obs_id and scan_mode
% criteria
\end{verbatim}
}% end \small
